...
class Environment
{
	...
	public function init()
	{
		try
		{
			...
			$cli = Kernel::set('core.args', new \Codebite\Yukari\CLI\CLIArgs($_SERVER['argv']));
			...
			$ui = Kernel::set('core.ui', new \Codebite\Yukari\CLI\UI());
			...
			Kernel::set('core.starttime', new \DateTime('@' . \Codebite\Yukari\START_TIME, $timezone));

			// Load the socket interface
			$socket = Kernel::set('core.socket', new \Codebite\Yukari\Connection\Socket());

			// Load the request map and response map
			$request_map = Kernel::set('core.request_map', new \Codebite\Yukari\Connection\RequestMap());
			$response_map = Kernel::set('core.response_map', new \Codebite\Yukari\Connection\ResponseMap());

			// Load any addons we want.
			$addon_loader = Kernel::set('core.addonloader', new \Codebite\Yukari\Addon\Loader());
	...
	public function runBot()
	{
		...
		try
		{
			...
			while(true)
			{
				...
				$ev = $dispatcher->trigger(\OpenFlame\Framework\Event\Instance::newEvent('runtime.tick'));
				$queue = (array) $ev->getReturns();
